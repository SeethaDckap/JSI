...
class CustomerIndex implements CustomerIndexRepositoryInterface
{
    ...
    public function getCustomers($searchCriteria)
    {
        ...
        foreach ($customers as $customer) {
            ...
            $this->dataObjectHelper->populateWithArray(
                $customerDTO,
                $customer,
                \Cloras\Base\Api\Data\CustomerInterface::class
            );
    ...
    public function saveCustomerIndex($customerId, $websiteId)
    {
        ...
        if (!$this->registry->registry('ignore_customer_update')) {
            ...
            if (empty($this->getERPCustomerId($customerId))) {
                $this->save($customer);
            }
     ...
     * @return string
     */
    public function getERPCustomerId($customerId)
    {
        $erpCustomerId = "";
        if ($customerId != 0) {
            ...
            if (is_array($customers->getData())) {
                if (array_key_exists('cloras_erp_customer_id', $customers->getData())) {
                    $erpCustomerId = $customers->getData()['cloras_erp_customer_id'];
                }
            }
        }

...
class Orders implements OrderResultsInterface
{
        ...
        if ($loggedOrders->getTotalCount()) {
            ...
            if (!empty($customerIds)) {
                ...
                foreach ($customers as $customer) {
                    ...
                    if (!$this->customerIndexRepository->getERPCustomerId($customer->getId())) {
                        ++$totalCustomers;
                    }
        ...
        //print_r($newOrder);exit();
                ...
                if (!empty($erpOrderId)) {
                    ...
                    /*$this->scopeConfig->getValue(
                      ...
                      exit();*/
                    //send email after updating erp order id
                    if ($orderId) {
    ...
    private function updateCustomerAttribute($newCustomer)
    {
        ...
        $customer->setCustomAttribute('cloras_erp_customer_id', $newCustomer['customer_id']);
        $customer->setCustomAttribute('cloras_erp_contact_id', $newCustomer['contact_id']);
        $customer->setCustomAttribute('cloras_erp_shipto_id', $newCustomer['shipto_id']);
    ...
    private function processOrderInfo(
        ...
        foreach ($ordersInfo as $orderInfo) {
            ...
            $status     = 'failure';
            $erpOrderId = 0;
            if (array_key_exists('erp_order_id', $orderInfo) && array_key_exists('web_reference_id', $orderInfo)) {
                $webrefId = $orderInfo['web_reference_id'];
                $erpOrderId = $orderInfo['erp_order_id'];
                $orderId = $this->getOrderId($webrefId, $orderInfo);
                ...
                if ($orderId) {
                    ...
                    $message .= ' ERP Order Id is not found';
                }//end if

                $order_status[$erpOrderId]['status']  = $status;
                $order_status[$erpOrderId]['message'] = $message;
                $this->orderIndexCollection->create()->updateSalesOrderId(
                    $erpOrderId,
                    $orderId
                );
                ...
                if (!array_key_exists('carrier_code', $orderInfo)) {
                    $message .= ' ERP Order Id or Web Reference Id Key is not found';
                }
            }//end if
        }//end foreach
    ...
    private function getOrderId($webrefId, $orderInfo)
    {  
    ...
    private function prepareOrderLineItems($order, $value, $matchedOrderItems, $status, $message, $orderType)
    {
        ...
        foreach ($order->getAllItems() as $orderItem) {
            ...
            elseif($orderType == 'invoice') {
                if (! $orderItem->getQtyToInvoice()) {
                    continue;
                }
            }

            $sku = ($orderItem->getExtOrderItemId() ? $orderItem->getExtOrderItemId() : $orderItem->getId());
        ...
        if (array_key_exists('tracking_number', $shipmentsData) && array_key_exists('carrier_code', $shipmentsData)) {
            if (!empty($shipmentsData['tracking_number']) && !empty($shipmentsData['carrier_code'])) {
                ...
                if (!empty($carrierCodeData)) {
                    ...
                    $carrierCode = '';
                    if (isset($carrierCodeData[$shipmentsData['carrier_code']])) {
                        $findcarrierCode = array("freeshipping_freeshipping"=>"custom","shipperadmin_adminshipping"=>"custom","shqfedexG_FEDEX_GROUND"=>"fedex","shqfedex_FEDEX_2_DAY"=>"fedex","shqfedex_FIRST_OVERNIGHT"=>"fedex","shqfedex_PRIORITY_OVERNIGHT"=>"fedex","shqfedex_STANDARD_OVERNIGHT"=>"fedex","shqfedex_GROUND_HOME_DELIVERY"=>"fedex","shqfedex_FEDEX_GROUND"=>"fedex");
                        $carrierCode = $findcarrierCode[$carrierCodeData[$shipmentsData['carrier_code']]];

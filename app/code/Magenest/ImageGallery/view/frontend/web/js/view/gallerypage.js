define([
    ...
    return function (config) {
                ...
                success: function (response) {
                    ...
                    if (response.length < 8) {
                        for (var i = 0; i < response.length; i++) {
                            ...
                            var image_size_factor = list_image[i].image_size_factor;
            ...
            if (deltaY > 0 && ($(window).scrollTop() > $(document).height() - $(window).height() - $('.page-footer').height() - $('.copyright').height())) {
                ...
                if (list_image.length < 4) {
                    for (var i = 0; i < list_image.length; i++) {
                        ...
                        console.log(list_image[i].image_size_factor+':'+imageId)
                        var word;
                        if (imageLove > 1)
                            word = '<span class="word">likes</span>';
                        else word = '<span class="word">like</span>';
                        $('body').addClass('image-loading')
                        appendImage(imageUrl,imageId,imageLove,color,product_id,word,image_size_factor);
                    ...
                    for (var i = 0; i < 4; i++) {
                        ...
                        var image_size_factor = list_image[0].image_size_factor;
                        console.log(list_image[0].image_size_factor+':'+imageId)
                        var word;
                        if (imageLove > 1)
                            word = '<span class="word">likes</span>';
                        else word = '<span class="word">like</span>';
        ...
        function appendImage(imageUrl,imageId,imageLove,color,product_id,word,image_size_factor)
        {
            $('#instagram-type').append('<div class="widget-timeline gallery-item grid-item '+ image_size_factor +' " id="image_' + imageId + '">\n' +

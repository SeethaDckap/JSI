 ...
 */
namespace Epicor\AccessRight\Acl\AclResource\Config\Converter;
...
class Dom implements \Magento\Framework\Config\ConverterInterface
{
    ...
    protected function _convertResourceNode(\DOMNode $resourceNode)
    {
        ...
        $displaytypeNode = $resourceAttributes->getNamedItem('displaytype');
        if ($displaytypeNode !== null) {
            $resourceData['displaytype'] = $displaytypeNode->nodeValue;
        }

        $actioncontolerNode = $resourceAttributes->getNamedItem('actioncontoler');
        if ($actioncontolerNode !== null) {
            $resourceData['actioncontoler'] = $actioncontolerNode->nodeValue;
        }

        $actioncodeNode = $resourceAttributes->getNamedItem('actioncode');
        if ($actioncodeNode !== null) {
            $resourceData['actioncode'] = $actioncodeNode->nodeValue;
        }

...
require([
	...
	$(document).ready(function() {
		var converter = $(document).find(".epr-treeview-converter");
		if (converter) {
			...
			var tree_html = '<div id="treeview_container" class="epr-treeview" style="height: ' + converter_height  +'; overflow-y: ' + converter_scroll + ';">' +
				'<ul id="treeview" class="epr-base">';


			//get treeview elements
			var tree = $(".epr-treeview-converter").children("li");
			...
			$.each(tree, function(i,e) {
				...
				if (numHyphen < numHyphen_next) {
					...
					item = item + '<label>' + "\n";
					item = item + '<input id="' + id_str  + '" data-id="' + data_id + '" class="chkbox" type="checkbox" /> ' + treeText;
					item = item + '</label>' + "\n";
					item = item + '<ul>' + "\n";
					//console.log(item);
				}
				...
				if (numHyphen == numHyphen_next) {
					item = item + '<li>' +"\n";
					item = item + '<label>' + "\n";
					item = item + '<input class="epr-end-node chkbox" id="' + id_str + '" data-id="' + data_id + '" type="checkbox" /> ' + treeText;
					item = item + '</label>' + "\n";
					item = item + '</li>' + "\n";
					//console.log(item);
				}
				//this is still a epr-end-node
			...
			if (allowed == true) {
				$(".epr-treeview-converter").after(tree_html);
			} else {
				$(".epr-treeview-converter").after(msg);
			}
	...
	$.fn.epr = function(options){


		var methodName = options;
		var args = arguments;
		var options = $.extend( {}, $.fn.epr.defaults, options);
		//initialisation
		if (typeof(methodName) == "undefined" ) {
			return this.each(function(){
				...
				if (options.checkboxesGroups == "disabled") {
					...
					var groups = $(this).find('.chkbox:not(".epr-end-node")');
				...
				if (options.collapseAll === false) {
					$.fn.epr.expandAll($(this),options.collapsedSymbol,options.expandedSymbol);
				}
				//-------------------options-------------------------------------------------------//
				...
				if (options.checkDoubles) {
					$(this).find('.chkbox.epr-end-node').each(function() {
				...
				//three state logic
				//$.fn.epr.ThreeStateLogic($(this),doubleMode,allVariables,options.checkDoubles,options.checkDisabled);
				$.fn.epr.ThreeStateLogic($(this),doubleMode,allVariables,options.checkDoubles,checkDisabled);

				//expandSingle
				$(this).on("click", 'li i.' + options.collapsedSymbol, function() {
					$.fn.epr.expandSingle($(this),options.collapsedSymbol,options.expandedSymbol);
				});
				//collapseSingle
				$(this).on("click", 'li i.' + options.expandedSymbol, function() {
					$.fn.epr.collapseSingle($(this),options.collapsedSymbol,options.expandedSymbol);
				});
			});
		}
		...
		if (methodName == "disableNode") {
			return this.each(function(){
				...
				$.fn.epr.disableNode($(this),attr,name,state,disableChildren);
			});
		}
		...
		if (methodName == "enableNode") {
			return this.each(function(){
				...
				$.fn.epr.enableNode($(this),attr,name,state,enableChildren);
			});
		}
		...
		if (methodName == "checkNode") {
			return this.each(function(){
				...
				$.fn.epr.checkNode($(this),attr,name);
				if (expandParents == true) {
					$.fn.epr.expandNode($(this),attr,name,expandParents,options.collapsedSymbol,options.expandedSymbol);
				}
			});
		}
		...
		if (methodName == "uncheckNode") {
			return this.each(function(){
				...
				$.fn.epr.uncheckNode($(this),attr,name);
				if (collapseChildren == true) {
					$.fn.epr.collapseNode($(this),attr,name,collapseChildren,options.collapsedSymbol,options.expandedSymbol);
				}
			});
		}
		...
		if (methodName == "setNodeColor") {
			return this.each(function(){
				...
				var ID = args[2];
				var color = args[3];
				$.fn.epr.setNodeColor($(this),attr,ID,color);
			});
		}
		...
		if (methodName == "collapseAll") {
			return this.each(function(){
				$.fn.epr.collapseAll($(this),options.collapsedSymbol,options.expandedSymbol);
			});
		}
		...
		if (methodName == "getChecked") {
			return this.each(function(){
				...
				$.fn.epr.getChecked($(this),attr,list,onlyEndNodes);
			});
		}
		...
		if (methodName == "getUnchecked") {
			return this.each(function(){
				...
				var list = args[1].list;
				var onlyEndNodes = args[1].onlyEndNodes;
				$.fn.epr.getUnchecked($(this),attr,list,onlyEndNodes);
			});
		}
		...
		if (methodName == "search") {
			return this.each(function(){
				...
				$.fn.epr.search($(this),treeview_container,search_input,search_output,search_button,dialog,enter_key_1,enter_key_2,options.collapsedSymbol,options.expandedSymbol,scrollOffset,onlyEndNodes,EnterKey);
			});
		}
	};
	...
	//-------------------checkAll---------------//
	$.fn.epr.checkAll = function(tree){
		tree.children("li").children("label").children(".chkbox").prop('indeterminate', false).css( "background-color", "#ffffff").prop('checked', false).trigger("click");
	};

	//-------------------uncheckAll---------------//
	$.fn.epr.uncheckAll = function(tree){
		//console.log(tree.children("li").children("label").children(".chkbox"))
		//find disabled groups
		var disabled_groups = tree.find('.chkbox:disabled:not(.epr-end-node)');
		...
		tree.children("li").children("label").children(".chkbox").prop('indeterminate', false).css( "background-color", "#ffffff").prop('checked', true).trigger("click");
	...
	//-------------------collapseAll---------------//
	$.fn.epr.collapseAll = function(tree,collapsedSymbol,expandedSymbol){
		tree.find("ul").hide();
		tree.find('.' + expandedSymbol).removeClass(expandedSymbol).addClass(collapsedSymbol);
	};

	//------------------expandAll------------------//
	$.fn.epr.expandAll = function(tree,collapsedSymbol,expandedSymbol){
		tree.find("ul").show();
		tree.find('.' + collapsedSymbol).removeClass(collapsedSymbol).addClass(expandedSymbol);
	};

	//-------------------collapseSingle---------------//
	$.fn.epr.collapseSingle = function(node,collapsedSymbol,expandedSymbol){
		node.parent("li").children("ul").hide();
		node.removeClass(expandedSymbol).addClass(collapsedSymbol);
	};

	//-------------------expandSingle---------------//
	$.fn.epr.expandSingle = function(node,collapsedSymbol,expandedSymbol){
		node.parent("li").children("ul").show();
		node.removeClass(collapsedSymbol).addClass(expandedSymbol);
	};

	//-------------------expandNode---------------//
	$.fn.epr.expandNode = function(tree,attr,name,expandParents,collapsedSymbol,expandedSymbol){
	...
	//-------------------collapseNode---------------//
	$.fn.epr.collapseNode = function(tree,attr,name,collapseChildren,collapsedSymbol,expandedSymbol){
		var that_node = tree.find('input[' + attr + '=' + name + ']');
		var that_ul = that_node.parent("label").siblings("ul");
	...
	$.fn.epr.checkNode = function(tree,attr,name){
		tree.find('.chkbox:not(:checked)[' + attr + '=' + name + ']').prop("indeterminate",false).css( "background-color", "#ffffff").trigger("click");
	};

	//-------------------uncheckNode---------------//
	$.fn.epr.uncheckNode = function(tree,attr,name){
		tree.find('.chkbox:checked[' + attr + '=' + name + ']').prop("indeterminate",false).css( "background-color", "#ffffff").trigger("click");
	};

	//-------------------disableNode---------------//
	$.fn.epr.disableNode = function(tree,attr,name,state,disableChildren){
		var this_checkbox = tree.find('.chkbox:not(:disabled)[' + attr + '=' + name + ']');
		...
		if (disableChildren === true) {
			this_checkbox.parent("label").parent("li").find('.chkbox').prop("disabled",true).parent("label").parent("li").css({'color':'#c8c8c8',"cursor":"not-allowed"});
		} else {
	...
	//-------------------enableNode---------------//
	$.fn.epr.enableNode = function(tree,attr,name,state,enableChildren){
		var this_checkbox = tree.find('.chkbox:disabled[' + attr + '=' + name + ']');
		...
		var children_not_disabled_sum= this_checkbox.parent("label").next("ul").children("li").children("label").children(".chkbox:not(:disabled)").length;
		...
		if (enableChildren === true) {
			this_checkbox.parent("label").parent("li").find('.chkbox').prop("disabled",false).parent("label").parent("li").css({'color':'#636b6f',"cursor":"default"});
		}
		this_checkbox.prop("checked",state === false);
		nodeEnabled = true;
		this_checkbox.trigger("click");
	...
	$.fn.epr.getChecked = function(tree,attr,list,onlyEndNodes){
		if (onlyEndNodes == true) {
			tree.find('.chkbox.epr-end-node:checked').each(function() {
				if (attr == "text") {
			...
			tree.find('.chkbox:checked').each(function() {
				if (attr == "text") {
					list.push($(this).parent("label").parent("li").text());
				} else {
					list.push($(this).attr(attr));
	...
	$.fn.epr.getUnchecked = function(tree,attr,list,onlyEndNodes){
		if (onlyEndNodes == true) {
			tree.find('.chkbox.epr-end-node:not(:checked)').each(function() {
				list.push($(this).attr(attr));
			});
		} else {
			tree.find('.chkbox:not(:checked)').each(function() {
				list.push($(this).attr(attr));
			});
		}
	};
	...
	//-------------------setNodeColor---------------//
	$.fn.epr.setNodeColor = function(tree,attr,ID,color){
		tree.find('.chkbox[' + attr + '=' + ID + ']').parent("li").css({'color':color});
	};

	//--------------three-state-logic----------------------//
	$.fn.epr.ThreeStateLogic = function(tree,doubleMode,allVariables,checkDoubles,checkDisabled) {
		tree.find('.chkbox').on('click', function(e) {
			...
			//all not disabled
			var nodes_below_not_disabled = $(this).parent("label").parent("li").find(".chkbox:not(:disabled)");

			//all disabled without epr-end-node
			var nodes_below_disabled_groups = $(this).parent("label").parent("li").find('.chkbox:disabled:not(.epr-end-node)');
			...
			if ($(this).prop("checked")) {
				var state = true;
				var checkSiblings = ".chkbox:not(:checked)";
				//fire event
				tree.trigger("nodeChecked",ids.join());
			} else {
				var state = false;
				var checkSiblings = ".chkbox:checked";
				//fire event
				tree.trigger("nodeUnchecked",ids.join());
			}
			//check / uncheck all checkboxes below that node
			nodes_below.prop("indeterminate",false).css( "background-color", "#ffffff").prop("checked",state);
			...
			$(this).parent("label").parent().parents("li").children("label").children(".chkbox").prop("indeterminate",true).css( "background-color", "#d5d5d5");
			$(this).parent("label").parent().parents("li").children("label").children(".chkbox").prop("checked",false);
			...
			$(this).parent("label").siblings("ul").find("ul").map(function(){
				...
				//console.log($(this).parent("label").next("ul").children("li").children("label").children(".chkbox"))
				//var not_disabled_sum_children = $(this).parent("label").next("ul").children("li").children("label").children(".chkbox:not(:disabled)").length;
				var disabled_sum_children = $(this).children("li").children("label").children(".chkbox:disabled").length;
				...
				var checked_sum_children = $(this).children("li").children("label").children(".chkbox:checked").length;
				var unchecked_sum_children = $(this).children("li").children("label").children(".chkbox:not(:checked)").length;
				...
				if (disabled_sum_children > 0) {
					if (checked_sum_children == 0) {
						$(this).siblings("label").children(".chkbox").prop("checked",false);
					}
					if (unchecked_sum_children == 0) {
						$(this).siblings("label").children(".chkbox").prop("checked",true);
					}
					if (checked_sum_children > 0 && unchecked_sum_children > 0) {
						$(this).siblings("label").children(".chkbox").prop("checked",false).prop("indeterminate",true).css( "background-color", "#d5d5d5");
					}
				}
			...
			$(this).parent("label").parents("li").map(function() {
				...
				if (checkDisabled) {
					var not_disabled_sum = $(this).siblings().addBack().children("label").children(".chkbox:not(:disabled)").length;
				...
				//var not_disabled_sum = $(this).siblings().addBack().children("label").children(".chkbox:not(:disabled)").length;
				//console.log("not_disabled_sum= " + not_disabled_sum);
				$(this).siblings().addBack().children("label").children(".chkbox").map(function() {
				...
				if ((indeterminate_sum + checked_unchecked_sum) == 0) {
					$(this).parent().parent().children("label").children(".chkbox").prop("indeterminate",false).css( "background-color", "#ffffff");
					$(this).parent().parent().children("label").children(".chkbox").prop("checked",state);
				}
			...
			if (checkDoubles == true && uncheckAll_doubles == false) {
				if (doubleMode == false) {
					...
					$(this).parent("label").parent("li").find("input.epr-end-node[type='checkbox']").each(function() {
			...
			//--------------------------disabled-----------------------------------------//
			//check if this box has epr-end-node children
			if (checkDisabled) {
				if ($(this).hasClass("epr-end-node") === false) {
					//if this box has been checked, check if "not checked disabled" exist
					if (state === true) {
						var disabledCheckboxes = $(this).parent("label").parent("li").find(".chkbox:not(:checked):disabled");
						var num_state_inverse_Checkboxes = $(this).parent("label").parent("li").find(".chkbox:checked");
					}
					//if this box has been unchecked, check if "checked disabled" exist
					if (state === false) {
						var disabledCheckboxes = $(this).parent("label").parent("li").find(".chkbox:checked:disabled");
						var num_state_inverse_Checkboxes = $(this).parent("label").parent("li").find(".chkbox:not(:checked)");
					}
					...
					if (disabledCheckboxes.length > 0 && num_state_inverse_Checkboxes.length > 0) {
						...
						disabledCheckboxes.parent("label").parent("li").parents("li").children("label").children(".chkbox:not(:disabled)").prop("indeterminate",true).css( "background-color", "#d5d5d5").prop("checked",state);
					}
				}
			}
			//--------------------------disabled-----------------------------------------//
	...
	//----------------------------search--------------------------------------//
	$.fn.epr.search = function(tree,treeview_container,search_input,search_output,search_button,dialog,enter_key_1,enter_key_2,collapsedSymbol,expandedSymbol,scrollOffset,onlyEndNodes,EnterKey) {
		...
		$(dialog + " #" + search_button).on("click", function(e) {
			...
			if (onlyEndNodes == true) {
				var onlyEndNodes_Class = ".epr-end-node";
			} else {
				var onlyEndNodes_Class = "";
			}
			tree.find('.chkbox' + onlyEndNodes_Class).each(function() {
			...
			$(dialog + " #" + search_output + " li").on("click", function(e) {
				...
				//before jumping to the epr-end-node a collapse all is needed
				tree.epr("collapseAll");
				//get this checkbox

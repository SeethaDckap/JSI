 ...
 */
namespace Epicor\BranchPickup\Block\Pickup\Select\Grid\Renderer;
            ...
            if ($row->getCode() == $getSelectedBranch) {
                ...
                foreach ($actions as $action) {
                    if (is_array($action)) {
                        if (($html != '') && ($action['caption'] != 'Select')) {
                            ...
                            $html .= '<a data-customid="'.$locationId.'" data-custom="'.$locationCode.'" class="selectBranchlink" id="selectBranchlink" href="#">'.$action['caption']->getText().'</a>';
                            if($error) {
                ...
                foreach ($actions as $action) {
                    if (is_array($action)) {
                        ...
                        $locationCode = $row->getData($this->getColumn()->getIndex());
                        $locationId = $row->getData('id');
                        $html .= '<a data-custom="'.$locationCode.'" data-customid="'.$locationId.'" id="selectBranchlink" class="selectBranchlink" href="#">'.$action['caption']->getText().'</a>';
                        if($error) {
                            $html .= '<input type="hidden" value="'.$locationCode.'" id="errorBranchlink_'.$locationId.'" class="errorBranchlink" >'; 
                        }                        
                    }

...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    {
        if ($this->registry->registry('access_group_data') && $this->registry->registry('access_group_data')->getId()) {
            $title = $this->registry->registry('access_group_data')->getEntityName();
            //   $title= Mage::app()->getLocale()->getCountryTranslation($title);
            //M1 > M2 Translation Begin (Rule 55)
            //return __('Edit Access Group "%s"', $this->htmlEscape($title));
            return __('Edit Access Group "%1"', $this->htmlEscape($title));
            //M1 > M2 Translation End
        } else {
            return __('New Access Group');
        }
    }

...
class Erpaccounttype extends \Magento\Framework\Data\Form\Element\AbstractElement
{
    ...
    private function getLabelValue($accountTypeValue, $accountTypeInfo = null)
    {
        ...
        if (!empty($accountTypeInfo)) {
            ...
            if (isset($accountTypeInfo['field'])) {
                ...
                if ($customer) {
                    $accountId = $accountTypeInfo['field'] === 'ecc_erpaccount_id' ? $customer->getEccErpAccountId() : $customer->getData($accountTypeInfo['field']);
                }
            }
        }
    ...
    protected function getAccountTypeHtml($baseFieldName, $type, $info, $customer = null)
    {
        ...
        $accountId = ($info['field'] === 'ecc_erpaccount_id' && $customer) ? $customer->getEccErpAccountId() : $accountId;
        ...
        $typesHtml .= '<input type="hidden" name="' . $type . '_url" id="' . $this->getHtmlId() . '_' . $type . '_url" value="' . $this->url->getUrl($info['url']) . '" />';
        ...
        if (empty($this->_accountType)) {
            ...
            if (empty($this->_accountType) && $customer) {
                ...
                foreach ($sortedTypes as $type) {
                    if (isset($type['field'])) {
                        $accountId = $type['field'] !== 'ecc_erpaccount_id' ? $customer->getData($type['field']) : $customer->getEccErpaccountId();
                        if (!empty($accountId)) {

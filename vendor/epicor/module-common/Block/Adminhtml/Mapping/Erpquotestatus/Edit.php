...
class Edit extends \Epicor\Common\Block\Adminhtml\Mapping\DefaultBlock\Edit
{
        ...
        array $data)
    {
        ...
        $this->_objectId = 'id';
        $this->_controller = 'adminhtml\Mapping_Erpquotestatus';
    ...
    {
        if ($this->registry->registry('erpquotestatus_mapping_data') && $this->registry->registry('erpquotestatus_mapping_data')->getCode()) {
            $title = $this->registry->registry('erpquotestatus_mapping_data')->getCode();
            //M1 > M2 Translation Begin (Rule 55)
            //return __('Edit Mapping "%s"', $this->htmlEscape($title));
            return __('Edit Mapping "%1"', $this->htmlEscape($title));
            //M1 > M2 Translation End
        } else {
            return __('New Erp Quote Status Mapping');
        }
    }

...
class Collection extends \Magento\Framework\Data\Collection
{
    ...
    protected $_dataFilters = array();
    protected $_filterString = '';
    protected $_data = array();
    protected $_helper = false;
    protected $_keepRowObjectType = false;
    protected $_dataSubset;
    protected $_idColumn;
    protected $_gridId;
    protected $_cacheKeyBase;
    protected $_cacheEnabled;
    protected $_cacheTime;
    protected $_showAll = false;
    protected $_additionalFilters;
    protected $_forceLinqSearch;
    protected $_maxResults;
    protected $_erpSearchEnabled;
    protected $_rowFilters;
    protected $_nFilters;
     ...
     * @return array
     */
    protected function _processFilter($filter, $force = false)
    {
        ...
        try {
            ...
            if (!$cache && !$session) {
                foreach ($results as $x => $row) {
                    ...
                    if ($this->getRowFilters()) {
                        $rowFilters = $this->getRowFilters();
                        foreach ($rowFilters as $_filter => $_filterValue) {
                            if (!is_array($_filterValue)) {
                                //filteration for CUID messages with miscellaneous charges
                                if($_filter === 'miscCharge'){
                                    $attributes = $row->getData('_attributes');
                                    if(!is_null($attributes->getMiscCharge()) && $attributes->getMiscCharge() === $_filterValue){
                                        unset($results[$x]);
                                    }
                                }else{
                                    if (($_filterValue == 'Y'  && is_null($row->getData($_filter)))
                                        || ($_filterValue == 'N'  && !is_null($row->getData($_filter)))
                                    ) {
                                        unset($results[$x]);
                                    }
                                }
                            }else{
                                // For DCLS filtering for dealerconnect dashboard
                                if(!in_array($row->getData($_filter), $_filterValue)){
                                    unset($results[$x]);
                                }
                            }
                        }
     ...
     * @return array
     */
    protected function _loadCachedData()
    {
     ...
     * @param string $type
     */
    protected function _storeData($data, $type)
    {
        if ($this->_cacheEnabled) {
            //M1 > M2 Translation Begin (Rule p2-6.7)
            //$cache = Mage::app()->getCacheInstance();
...
//        }
     ...
     */
    protected function _searchData($results)
    {
    ...
    {
        $this->load();
        if ($this->_totalRecords === null && $this->getItems()) {

...
/* global varienGrid, setLocation, varienGlobalEvents, FORM_KEY,
    FRONTEND_BASE_URL, Base64, varienGridMassaction, varienStringArray, serializerController
*/
...
define([
    ...
    'Epicor_Customerconnect/js/document_print/email-mass-popup',
...
], function (jQuery, mageTemplate, alert, confirm, popup, sendAjax, utils) {
    /**
     * @param {*} grid
     * @param {*} event
     */
    ...
    varienGrid.prototype = {
        ...
        doExport: function () {
            ...
            if ($(this.containerId + '_export')) {
                ...
                location.href = exportUrl + 'id/' + Date.now();
            }
        },
    ...
    varienGridMassaction.prototype = {
        ...
        emailForm: jQuery('form#email-massaction-popup-form'),
        transportParams: {email_params: {}},
        ...
        apply: function () {
            ...
            if(typeof this.getSelectedItem().url !== "undefined"){
                var max_count = utils.getUrlParameters(this.getSelectedItem().url).maxCount;
                if(varienStringArray.count(this.checkedString) > max_count){
                    alert({
                        content: jQuery.mage.__('Please select up to '+max_count+' items.')
                    });
            ...
            if(this.getSelectedItem().id === 'email' && this.getSelectedItem().url.indexOf('customerconnect/massactions') !== -1){
                var entity_document = utils.getUrlParameters(this.getSelectedItem().url).entity;
                popup.showPopup(entity_document);
            }else{
                this.formSubmit();
            }
        },
        ...
        onConfirm: function (fieldName, item) {
            ...
            var canUseAjax =  this.getSelectedItem().id === 'email' && this.getSelectedItem().url.indexOf('customerconnect/massactions') !== -1;

            if(canUseAjax && item.url){
                popup.closePopup();
                var formData = this.form.serialize(true);
                formData .email_params = jQuery.param(this.transportParams.email_params);
                formData.entity_document = utils.getUrlParameters(item.url).entity;
                new Ajax.Request(item.url, {
                    ...
                    'onSuccess': function(data) {
                        ...
                        window.parent.showMessage(json.message, json.type);
                        sendAjax.processPreq(json.id);
                    }.bind(this)
                });
                this.unselectAll();
                return;
        ...
        setEmailParams: function(){
            this.transportParams.email_params.to = this.emailForm.find('input#mass-print-email-to').val();
            this.transportParams.email_params.cc = this.emailForm.find('input#mass-print-email-cc').val();
            this.transportParams.email_params.bcc = this.emailForm.find('input#mass-print-email-bcc').val();
            this.transportParams.email_params.message = this.emailForm.find('textarea#mass-print-email-message').val();
            this.transportParams.email_params.subject = this.emailForm.find('input#mass-print-email-subject').val();
        },

...
class PaymentValue extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\Action
{
    ...
    public function render(\Magento\Framework\DataObject $row)
    {
        ...
        $invoiceAmount = $row->getPaymentValue();
        if($row->getSelectArpayments() !="Totals") {
            $html = '<span class="price">'.$helper->formatPrices($row->getPaymentValue()).'</span>';
        }  else {
            $html = '<span class="price"><strong>'.$helper->formatPrices($row->getPaymentValue()).'</strong></span>';
        } 
        return $html;
    }

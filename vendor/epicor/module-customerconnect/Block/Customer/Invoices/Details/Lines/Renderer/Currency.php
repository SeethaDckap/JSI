...
class Currency extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\AbstractRenderer
{
    ...
    public function render(\Magento\Framework\DataObject $row)
    {
        ...
        $currency = $helper->getCurrencyMapping($invoice->getCurrencyCode(), \Epicor\Comm\Helper\Messaging::ERP_TO_MAGENTO);
        if ($row->getData($index)) {
            return $helper->formatPrice($row->getData($index), true, $currency);
        } else if($index === 'miscellaneous_charges_total') {
            return $helper->formatPrice($row->getData(0), true, $currency);
        }
    }

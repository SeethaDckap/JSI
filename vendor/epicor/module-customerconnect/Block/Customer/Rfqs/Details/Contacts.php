 ...
 */
class Contacts extends \Epicor\Common\Block\Generic\Listing
{
    ...
    public function getButtonsHtml($region = null)
    {
        ...
        foreach ($this->buttons->getItems() as $buttons) {
            if (!array_key_exists('add_contact', $buttons) && (!array_key_exists('add_attachment', $buttons))) {

                continue;
            }
            unset($buttons['add_attachment']);
            /* remove add_salesrep button */

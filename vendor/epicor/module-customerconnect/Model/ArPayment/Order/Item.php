 ...
 * @method \Epicor\Customerconnect\Model\ArPayment\Order\Item setGiftMessageId(int $value)
 * @method int getGiftMessageAvailable()
 * @method \Epicor\Customerconnect\Model\ArPayment\Order\Item setGiftMessageAvailable(int $value)
...
class Item extends AbstractModel implements OrderItemInterface
{
    ...
    // When qty ordered = qty returned // not used at the moment
    protected $_eventPrefix = 'ecc_ar_sales_order_item';
    ...
    {
        $this->_init(\Epicor\Customerconnect\Model\ArPayment\ResourceModel\Order\Item::class);
    }
    ...
    public function cancel()
    {
        if ($this->getStatusId() !== self::STATUS_CANCELED) {
            $this->_eventManager->dispatch('ecc_ar_sales_order_item_cancel', ['item' => $this]);
    ...
    {
        if ($item instanceof \Epicor\Customerconnect\Model\ArPayment\Order\Item) {
            $this->_children[] = $item;

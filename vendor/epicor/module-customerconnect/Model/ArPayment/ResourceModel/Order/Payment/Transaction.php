 ...
 */
namespace Epicor\Customerconnect\Model\ArPayment\ResourceModel\Order\Payment;
...
class Transaction extends EntityAbstract
{
    ...
    {
        $this->_init('ecc_ar_sales_payment_transaction', 'transaction_id');
    }
     ...
     *
     * @param \Epicor\Customerconnect\Model\ArPayment\Order\Payment\Transaction $transaction
     * @return void
     */
    public function injectAsParent(\Epicor\Customerconnect\Model\ArPayment\Order\Payment\Transaction $transaction)
    {
        $txnId = $transaction->getTxnId();
        if ($txnId &&
            \Epicor\Customerconnect\Model\ArPayment\Order\Payment\Transaction::TYPE_PAYMENT === $transaction->getTxnType() &&
     ...
     * @param int $paymentId
     * @param string $txnId
     * @return \Epicor\Customerconnect\Model\ArPayment\Order\Payment\Transaction
     */
    public function loadObjectByTxnId(
        \Epicor\Customerconnect\Model\ArPayment\Order\Payment\Transaction $transaction,
        $orderId,
        $paymentId,
        $txnId
    ) {
    ...
    public function getOrderWebsiteId($orderId)
    {
        ...
        $select = $connection->select()->from(
            ['so' => $this->getTable('ecc_ar_sales_order')],
            'cs.website_id'
        )->joinInner(
     ...
     *
     * @param \Magento\Framework\Model\AbstractModel|\Epicor\Customerconnect\Model\ArPayment\Order\Payment\Transaction $transaction

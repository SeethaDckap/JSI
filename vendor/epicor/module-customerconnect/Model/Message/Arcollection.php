 ...
 */
class Arcollection extends \Magento\Framework\Data\Collection
{
    ...
    private $_includeYesItems = array();
    private $_includeNoItems = array();
    private $_includeNoSelectedItems = false;
    ...
    public function setFilterPageSize($pageItems)
    {
        $this->_pageSize = count($pageItems);
    }
    ...
    public  function setIncludeYesItems($value=null) {
        $this->_includeYesItems = $value;
    }
    
    public  function setIncludeNoItems($value=null) {
        $this->_includeNoItems = $value;
    }  
    
    public  function setIncludeNoSelectedItems($value=false) {
        $this->_includeNoSelectedItems = $value;
    }      
    ...
    private function _loadData()
    {
        ...
        try {
            ...
            if (!$cache && !$session) {
               $selectedYesItems = false;
               $selectedNoItems = false;
               $selectedNoContentItems = false; 
               $getSelectedItems = $this->request->getParam('invoices');
               $selectedFilter  = $this->request->getParam('filter');
                if (!is_null($selectedFilter)) {
                    $filter = $this->backendHelper->prepareFilterString($selectedFilter);
                    if ((isset($filter['select_arpayments'])) && ($filter['select_arpayments'] =="1") && !empty($getSelectedItems)) {
                        $selectedYesItems = $getSelectedItems;
                    }
                    if ((isset($filter['select_arpayments'])) && ($filter['select_arpayments'] =="0") && !empty($getSelectedItems)) {
                        $selectedNoItems = $getSelectedItems;
                    } 
                    if ((isset($filter['select_arpayments'])) && ($filter['select_arpayments'] =="1") && empty($getSelectedItems)) {
                        $selectedNoContentItems = true;
                    }                     
                }               
                ...
                foreach ($results as $x => $row) {
                    if (!is_null($row) && !empty($row)) {
                        ...
                        if((!empty($selectedYesItems)) && (in_array($row->getInvoiceNumber(), $selectedYesItems)))  {
                                $results[$x] = $row;
                        } else {
                           if(!empty($selectedYesItems)) {
                               unset($results[$x]);
                           }
                        }
                        
                        if((!empty($selectedNoItems)) && (!in_array($row->getInvoiceNumber(), $selectedNoItems)))  {
                            $results[$x] = $row;
                        } else {
                           if(!empty($selectedNoItems)) {
                               unset($results[$x]);
                           }
                        }
                        if($selectedNoContentItems)  {
                            unset($results[$x]);
                        } else {
                          if(!$selectedYesItems && !$selectedNoItems && !$selectedNoContentItems) {
                            $results[$x] = $row;  
                          }
                        }                           
                    ...
                    if(($this->_messageType === "crqs" || $this->_messageType === "cuos") && $this->request->getModuleName() === "dealerconnect" && is_null($row->getData('dealer'))){
                        unset($results[$x]);
                    }
                }
    ...
    private function _setItems()
    {
        ...
        try {
            ...
            foreach ($this->_orders as $key => $order) {
                ...
                $type = isset($columnInfo['type']) ? $columnInfo['type'] : 'text';

 ...
 */
namespace Epicor\Dealerconnect\Block\Adminhtml\Config\Form\Field\Dmau;
...
class InformationLayout extends \Magento\Config\Block\System\Config\Form\Field\FieldArray\AbstractFieldArray
{
    ...
    protected function _getMappingRenderer()
    {
        ...
        if (!$this->_mappingRenderer) {
            ...
            $this->_mappingRenderer->setInputName('index')->setClass('rel-to-selected dmauinfomappingrenderer');
        }
    ...
    protected function _getTypeRenderer()
    {
        if (!$this->_typeRenderer) {
            ...
            $this->_typeRenderer->setInputName('type')->setClass('rel-to-selected dmauinfotype');
        }
    ...
    protected function _getHiddenRenderer()
    {
        if (!$this->_hiddenRenderer) {
            ...
            $this->_hiddenRenderer->setInputName('hiddenpac')->setClass('rel-to-selected dmauhidden');
        }
    ...
    protected function _construct()
    {
        ...
        $this->setHtmlId('_dmauinformation');
        parent::_construct();
    }
    ...
    public function _toHtml()
    {
        ...
        $html .= '
            <style>
                .rel-to-selected.dmautype {width: 100% !important;}
                .rel-to-selected.dmaufilter_by {width: 100% !important;}
                #grid_dmauinformation table th:nth-child(3){ display:none;} 
                #grid_dmauinformation table td:nth-child(3){ display:none;}      
                #grid_dmauinformation table.admin__control-table .altdmau td {  }
                #grid_dmauinformation table.admin__control-table td { background-color:transparent ;  }
                #grid_dmauinformation table.admin__control-table { background-color: #f2f2f2; }
                #grid_dmauinformation table.admin__control-table th { background-color:#5f564f; color:#fff; }
                #grid_dmauinformation select {
                    font-size: 13px;
                }   
                #grid_dmauinformation tbody tr:hover {
                  background-color: #e5f7fe;
                }
                #grid_dmauinformation td {
                    padding-left: 2px !important;
                    padding-right: 5px !important;
                }   
                
                #grid_dmauinformation {
    ...
    protected function _renderValue(\Magento\Framework\Data\Form\Element\AbstractElement $element)
    {
        ...
        $html .='<table cellpadding="2" cellspacing="4" style="border: 1px solid burlywood;border-top:0px;margin-top: -6px;border-collapse: separate;border-radius: 5px;background: oldlace;"><tr><td>'
            ...
            . '<tr><td>'
            . '<select id="dmau_address_column" name="dmau_address_column" class=" select admin__control-select" data-ui-id="">'
            . '<option value="1" selected="selected">Column</option></select>'
            . '</td>'
            . '<td>'
            . '<select id="dmau_address_column_count" name="dmau_address_column_count" class=" select admin__control-select" data-ui-id="">'
            . '<option value="2" selected="selected">3</option></select>'
            . '</td>'
            . '<td>'
            . '<select id="dmau_address_column_section" name="dmau_address_column_section" class=" select admin__control-select" data-ui-id="">'

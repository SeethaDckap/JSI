...
class Confirm extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\AbstractRenderer
{
    ...
    public function render(\Magento\Framework\DataObject $row)
    {
        ...
        if (!$this->registry->registry('rfqs_editable') || $status != \Epicor\Customerconnect\Model\Config\Source\Quotestatus::QUOTE_STATUS_AWAITING  || !$currencyCode
        ) {
        ...
        $html = '<input type="checkbox" name="confirmed[]" value="' . $row->getId() . '" id="rfq_confirm_' . $row->getId() . '" class="rfq_confirm" ' . $disabled . '/>'
            ...
            . '<input type="hidden" name="rfq[' . $row->getId() . '][checkbox]" class="rfq_confirm_checkbox" value="' . $isDisabled . '"/>'
        ;

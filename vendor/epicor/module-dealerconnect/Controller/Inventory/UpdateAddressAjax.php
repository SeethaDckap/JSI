...
class UpdateAddressAjax extends \Epicor\Customerconnect\Controller\Rfqs {
    ...
    public function execute() 
    {
        ...
        $tag = $this->request->getParam('addresstype');
        ...
        if ($addressId) {
            ...
            $content = $result->getLayout()->createBlock('Epicor\Dealerconnect\Block\Portal\Inventory\Details\UpdateAddressAjax')->setAddressType($type)->setAddressFromCustomerAddress($address)->toHtml();
        } else {
            $addressParam = $this->request->getParam('address-data');
            $addressData = !empty($addressParam) ? (array) json_decode($addressParam) : array();

            $content = $result->getLayout()->createBlock('Epicor\Dealerconnect\Block\Portal\Inventory\Details\UpdateAddressAjax')
                            ->setAddressType($type)

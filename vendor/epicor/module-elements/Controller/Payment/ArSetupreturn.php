 ...
 */
class ArSetupreturn extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if ($elements->getId()) {
            $elements->setHostedPaymentStatus(@$data['HostedPaymentStatus'])->setHostedExpressResponseCode(
                    @$data['ExpressResponseCode']
                )->setHostedExpressResponseMessage(@$data['ExpressResponseMessage'])->setHostedServicesId(
                    @$data['ServicesID']
                )->setHostedValidationCode(@$data['ValidationCode'])->setCreditCardAuthHostResponseCode(
                    @$data['ExpressResponseCode']
                )->setCreditCardAuthHostResponseMessage(@$data['ExpressResponseMessage'])->setBillingAddress(
                    @$data['BillingAddress1']
                )->setBillingZipcode(@$data['BillingZipcode'])->setPaymentAccountId(
                    @$data['PaymentAccountID']
                )->setLastFour(@$data['LastFour'])->setAvsResponseCode(@$data['AVSResponseCode'])->setCvvResponseCode(
                    @$data['CVVResponseCode']
                )->setTransactionId(@$data['TransactionID'])->setApprovalNumber(
                    @$data['ApprovalNumber']
                )->setApprovedAmount(@$data['ApprovedAmount'])->setCardLogo(@$data['CardLogo'])->save();
            $log = '';
            if ($elements->successfulHostedResponse()) {
                ...
                if ($this->scopeConfig->isSetFlag('payment/elements/CVVEnabled', \Magento\Store\Model\ScopeInterface::SCOPE_STORE
                    ) && $elements->validateAvsResponse() && $elements->validateCvvResponse()) {
            ...
            if ($elements->hasError()) {
                $this->logger->error(
                    "TransactionSetupID".$elements->getTransactionSetupId()."Error".$elements->getTransactionSetupId().$log
                );
            }

...
Epicor_Lists.listProduct.prototype = {
    ...
    pricing: function (row, event) {
        ...
        let selectedRow = jQuery(rowSelector);
        let currentRow = selectedRow.find('td input.admin__control-checkbox');
        let priceGrid = currentRow.next('tr');
        let gridVisible = !!priceGrid.is(":visible");

        if (currentRow.prop('checked') === false && !gridVisible) {
            currentRow.trigger('click');
            let positionInput = selectedRow.find('td.ecc-list-position input');
    ...
    import: function () {
        ...
        if ($('import')) {
            ...
            var deleteAllPrice = $('deleteAllProducts').value;
            var data = new FormData();

            if (files.length > 0) {
                if (files[0].name.split('.').pop().toLowerCase() != 'csv') {
                    alert(this.translate('Wrong File Type. Only CSV files are allowed.'));
                } else {
                    if(!confirm(jQuery.mage.__('The import will overwrite the existing data and cannot be undone.'))) {
                        return ;
                    }
                    data.append('import', files[0], files[0].name);
                    data.append('deleteAllProducts', deleteAllPrice);
                    ...
                    jQuery.ajax({
                        ...
                        success: function (response) {
                            ...
                            _this.products = {};//After import refresh the product pricing index
    ...
    reloadGrid: function (products) {
        ...
        gridObject.reload(null, function () {
            //Update Grid Product Posting
            jQuery('#productsGrid').trigger('positionUpdate');
        });
    },
    serializeProducts: function (products) {
        ...
        for (var i = 0; i < products.length; i++) {
            var id = products[i];
            array.push(encodeURIComponent(id) + "=" + encodeURIComponent(btoa('id=' + id)));
        }
        return array.join('&');
    },
    dowloadCsv: function () {
...
{
    jQuery('#productsGrid_table input[type=checkbox]').each(function () {

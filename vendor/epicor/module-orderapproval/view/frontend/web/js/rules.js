...
define([
    ...
    jQuery(document).on('click','a.approval-limit', function(){
        let parentListElement = jQuery('li span.rule-param-new-child').closest('li');
        if(parentListElement){
            parentListElement.show();
        }
    });
    ...
    VarienRulesForm.prototype = {
        initialize: function (parent, newChildUrl) {
            ...
            this.hideSelectorForApprovalLimit();
        ...
        hideSelectorForApprovalLimit: function(){
            let totalInput = jQuery('#conditions__1--1__value');
            if(totalInput && totalInput.length > 0){
                let parentListElement = jQuery('li span.rule-param-new-child').closest('li');
                if(parentListElement){
                    parentListElement.hide();
                }
            }
        },
 ...
 i;
            ...
            var groupId = jQuery('input#group-id-val').val();

            new Ajax.Request(this.newChildUrl, {
                evalScripts: true,
                parameters: {
                    form_key: FORM_KEY, type: new_type.replace('/', '-'), condition_id: new_id, id: groupId
                },

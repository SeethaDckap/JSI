...
(function() {
    ...
    function Linechart(paper, x, y, width, height, valuesx, valuesy, opts) {
        ...
        function createDots(f) {
            ...
            for (var i = 0, ii = valuesy.length; i < ii; i++) {
                for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
                    ...
                    f ? (C = {}) : cvrs.push(C = paper.circle(X, Y, 7).attr({stroke: "none", fill: "#000", opacity: 0}));
                    C.x = X;
                    C.y = Y;
                    C.value = valuesy[i][j];
        ...
        var legend = function(legends, mark, dir) {

            legends = legends || [];

            var cx = x / 5,
                    h = y + 50 + (legends['offset'] || 0);
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                legends[i] = chartinst.labelise(legends[i], 155, 2);
                chart.labels.push(paper.set());
                chart.labels[i].push(paper[mark](x + 5, h, 5).attr({fill: lines[i].attr('stroke'), stroke: lines[i].attr('stroke')}));
                chart.labels[i].push(txt = paper.text(x + 20, h, legends[i] || valuesx[j]).attr(chartinst.txtattr).attr({fill: opts.legendcolor || "#000", "text-anchor": "start"}));
                h += txt.getBBox().height + gutter;
            }
            ...
            var bb = chart.labels.getBBox(),
                    tr = {
                        ...
                        north: [(width + gutter - bb.width) / 2, 0],
                        south: [(width + gutter - bb.width) / 2, height + gutter]
                    }[dir];
        ...
        if (opts.legend) {
            legend(opts.legend, opts.legendmark, opts.legendpos);
        }
